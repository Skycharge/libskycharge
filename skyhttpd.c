...
static void
httpd_queue_empty_response(struct httpd *httpd, struct MHD_Connection *con,
			   int http_status)
{
	...
	(void)httpd_queue_response(httpd, con, http_status, resp);
...
httpd_expire_addrs_and_requests(struct httpd *httpd, struct timeval *tv)
{
	...
	while ((node = rb_first(&httpd->reqs_root))) {
		...
		httpd_queue_empty_response(req->httpd, req->httpcon,
